...
class EMA:
    ...
    def restore(self, model):
        """Optional: restore model weights to non-EMA ones."""
        for name, param in model.named_parameters():
            if param.requires_grad:
                self.shadow[name] = param.data.clone()
...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument('--seed', type=int, default=123, help='random seed') #47
    ...
    parser.add_argument("--warmup_proportion", default=0.5, type=float,
...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    decay_clip_param_tp = [(n, p) for n, p in decay_param_tp if "modelblip." in n]
    decay_noclip_param_tp = [(n, p) for n, p in decay_param_tp if "modelblip." not in n]

    no_decay_clip_param_tp = [(n, p) for n, p in no_decay_param_tp if "modelblip." in n]
    no_decay_noclip_param_tp = [(n, p) for n, p in no_decay_param_tp if "modelblip." not in n]

    weight_decay = 0.2
    optimizer_grouped_parameters = [
        {'params': [p for n, p in decay_clip_param_tp], 'weight_decay': weight_decay, 'lr': args.lr * 0.01},
        {'params': [p for n, p in decay_noclip_param_tp], 'weight_decay': weight_decay},
        {'params': [p for n, p in no_decay_clip_param_tp], 'weight_decay': 0.0, 'lr': args.lr * 0.01},
    ...
    #optimizer = SGD(model.parameters(), lr=1e-2, momentum=0.9, nesterov=True, weight_decay=1e-4)
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0, ema=None):
    global logger
    ...
    total_loss = 0
    print('test_params')
    print("total_parameters:" + str(count_parameters(model)))
    priint('end')
    for step, batch in enumerate(train_dataloader):
        if n_gpu == 1:
            ...
            pass
        key_frame, input_ids, input_mask, segment_ids, video, video_mask, bbox, ann, video_fast, inputs = batch
        #print("test")
        #print(video)
        key_frame = key_frame.cuda()


        #video = torch.cat([video, video_fast], dim=0)
        '''def closure():
            loss, predictions, bbox_predictions,_ = model(key_frame, input_ids[0], segment_ids, input_mask, video, video_mask, bbox, ann)
...
def eval_epoch(args, model, test_dataloader, device, n_gpu, test, ema=None):
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader): # Maybe something went wrong here!!!
            key_frame, iids, input_mask, segment_ids, video, video_mask, bbox, anns, video_fast, inputs = batch


            #key_frame, input_ids, input_mask, segment_ids, video, video_mask, bbox, ann, video_fast = batch

            #key_frame = key_frame.cuda()

            #video = torch.cat([video, video_fast], dim=0).cuda()
    ...
    if args.do_train:
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            tr_loss, global_step = train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer,
                                               scheduler, global_step, local_rank=args.local_rank, ema=ema)
            if args.local_rank == 0:
            ...
            #if epoch >5:
            R1 = eval_epoch(args, model, test_dataloader, device, n_gpu, test='EVAL', ema=ema)
            R2 = eval_epoch(args, model, val_dataloader, device, n_gpu, test='TEST', ema=ema)
