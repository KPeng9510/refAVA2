...
from dataloaders.ravar_dataset_pami_v1 import RAVARDataset
from torch.utils.data import Sampler
import numpy as np
class DistributedWeightedSampler(Sampler):
    """
    Deterministic, replacement sampling with weights across DDP ranks.
    - Draws a single global sequence with torch.multinomial and a shared seed.
    - Shards by rank to avoid overlap.
    """
    def __init__(self,
        ...
        self.num_data = self.weights.numel()
        self.replacement = replacement
        self.rank = rank
        self.world_size = world_size
        self.seed = seed

        # how many samples each rank should draw per epoch
        if num_samples_per_rank is None:
            # default: roughly one pass per replica
            self.num_samples_per_rank = math.ceil(self.num_data / world_size)
        else:
            self.num_samples_per_rank = int(num_samples_per_rank)

        # total draws across all ranks
        ...
        ws = self.weights.sum()
        if ws > 0:
            self.weights = self.weights / ws
        else:
            # fallback uniform if weights degenerate
            self.weights = torch.full_like(self.weights, 1.0 / self.num_data)

    def __iter__(self):
        g = torch.Generator()
        g.manual_seed(self.seed + self.epoch)

        # global draw, then stride to shard
        ...
        # shard by rank: i, i+world_size, ...
        indices = global_indices[self.rank:self.total_size:self.world_size]
...
def dataloader_ravar_train(args, tokenizer, subset="train"):
    ...
    '''y_multilabel = torch.Tensor(ravar_dataset.annotations)
    label_freq = torch.Tensor(y_multilabel.sum(0))                # [C], count of each label
    class_weights = 1.0 / (label_freq.float() + 1e-6)

    # sample weight = sum of weights for all labels the sample has
    sample_weights = (y_multilabel * class_weights).sum(1)

    sampler = DistributedWeightedSampler(
        weights=sample_weights,
        num_samples_per_rank=len(sample_weights)//args.n_gpu,
        replacement=True
    )'''
