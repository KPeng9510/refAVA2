...
class RAVARDataset(Dataset):
    ...
    def video_to_tensor(self, frames, preprocess, sample_fp=0, start_time=None, end_time=None):
        ...
        for frame in frames[0]:
            ...
            image = self.processor(images=Image.fromarray(frame.astype(np.uint8)).convert("RGB"), return_tensors="pt" )
            #print(image['pixel_values'].shape)
            #import os
            #os.exit()
            images.append(image['pixel_values'].squeeze())
        if len(images) > 0:
    ...
    def __getitem__(self, index):
        if self.training == 'train':
            ...
            return  key_frame, pairs_text, pairs_mask, pairs_segment, video, video_mask, bboxes, ann, video_fast
        else:
            start_frame = self.start_frames[index]
            ann = self.annotations[index]
            video_name = self.video_names[index]
        ...
        return  key_frame, pairs_text, pairs_masks, pairs_segments, videos, video_masks, bboxes, anns, videos_fast
