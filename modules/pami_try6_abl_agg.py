...
class LSTMAggregator(nn.Module):
    """
    Input : x of shape (B, N, C)
    Output: y of shape (B, N, D)
    """
    def __init__(
        self,
        in_dim: int,                 # C
        ...
        dropout: float = 0.1         # applied between LSTM layers (not on last if num_layers=1)
    ):
        super().__init__()
        self.bidirectional = bidirectional
        self.num_dirs = 2 if bidirectional else 1

        # Optionally lift to a higher rep before LSTM
        ...
        # Map LSTM output (num_dirs*hidden_dim) -> D
        self.out_proj = nn.Linear(self.num_dirs * hidden_dim, out_dim)

    @torch.no_grad()
    def _maybe_lengths(self, x: torch.Tensor, lengths: Optional[torch.Tensor]) -> torch.Tensor:
        if lengths is None:
            # assume all valid
    ...
    def forward(
        ...
        lengths: Optional[torch.Tensor] = None  # (B,) valid lengths per sample
    ) -> torch.Tensor:
        B, N, _ = x.shape

        x = self.input_proj(x)                  # (B, N, hidden_dim)

        lengths = self._maybe_lengths(x, lengths)
        # Sort by length for packing (required)
        ...
        out_sorted, _ = pad_packed_sequence(packed_out, batch_first=True, total_length=N)  # (B, N, num_dirs*hidden)
        ...
        y = self.out_proj(out)                  # (B, N, D)
        return y

class TransformerAggregator(nn.Module):
    """
    Aggregates token features with a Transformer encoder.
    Input:  x  of shape (B, N, C)
    Output: out of shape (B, N, D)
    """
    def __init__(
        self,
        in_dim: int,                 # C
        ...
        num_layers: int = 2,
        nhead: int = 8,
        dim_feedforward: int = None, # defaults to 4 * out_dim
        dropout: float = 0.1,
        max_len: int = 2048,         # max N you expect
        ...
        # Project input tokens from C -> D for the Transformer
        self.input_proj = nn.Linear(in_dim, out_dim)

        # (Optional) learnable positional embeddings
        self.use_positional_encoding = use_positional_encoding
        if use_positional_encoding:
            self.pos_embed = nn.Parameter(torch.zeros(1, max_len, out_dim))
            nn.init.trunc_normal_(self.pos_embed, std=0.02)

        # Standard Transformer encoder (expects shape (S, B, E))
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=out_dim,
            nhead=nhead,
            dim_feedforward=dim_feedforward,
            dropout=dropout,
            batch_first=False,  # we'll feed (S, B, E)
            activation="gelu",
            norm_first=True
        )
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)

        # (Optional) output projection (D -> D). Keep identity if not needed.
        self.output_proj = nn.Identity()

    def forward(
        self,
        x: torch.Tensor,                          # (B, N, C)
        key_padding_mask: Optional[torch.Tensor] = None  # (B, N) with True for PAD positions
    ) -> torch.Tensor:
        B, N, _ = x.shape
        x = self.input_proj(x)                    # (B, N, D)

        if self.use_positional_encoding:
            # Trim positional embeddings to current N
            x = x + self.pos_embed[:, :N, :]

        # nn.TransformerEncoder wants (S, B, E)
        x = x.transpose(0, 1)                     # (N, B, D)

        # key_padding_mask: (B, N) where True -> ignore
        out = self.encoder(x, src_key_padding_mask=key_padding_mask)  # (N, B, D)
        out = out.transpose(0, 1)                 # (B, N, D)

        out = self.output_proj(out)               # (B, N, D)
        return out
...
def focal_binary_cross_entropy_label_smooth(
    logits, targets, gamma=2.0, alpha=None, label_smoothing=0.0
):
    # flatten
...
class MixerBlock(nn.Module):
    ...
    One MLP-Mixer block.
    x: (B, N, D_model)
    - Token-mixing: mixes along N (per channel)
    - Channel-mixing: mixes along D_model (per token)
    """
    def __init__(self, n_tokens: int, d_model: int, token_mlp_dim: int, channel_mlp_dim: int, dropout: float = 0.0):
        super().__init__()
        self.ln_token = nn.LayerNorm(d_model)
        self.token_mlp = FeedForward(n_tokens, token_mlp_dim, n_tokens, dropout)

        self.ln_channel = nn.LayerNorm(d_model)
        self.channel_mlp = FeedForward(d_model, channel_mlp_dim, d_model, dropout)

    def forward(self, x: torch.Tensor, pad_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        # x: (B, N, D)
        y = self.ln_token(x)
        # token-mixing MLP runs across the token axis -> transpose to (B, D, N)
        y = y.transpose(1, 2)                   # (B, D, N)
        y = self.token_mlp(y)                   # (B, D, N)
        y = y.transpose(1, 2)                   # (B, N, D)
        x = x + y                                # residual

        # channel-mixing per token
        z = self.ln_channel(x)                  # (B, N, D)
        z = self.channel_mlp(z)                 # (B, N, D)
        out = x + z                             # residual

        # Optional: zero-out padded positions to keep them inert
        if pad_mask is not None:
            # pad_mask: (B, N) with True for PAD positions
            out = out.masked_fill(pad_mask.unsqueeze(-1), 0.0)
        return out

# ---------- Aggregator ----------

class MLPMixerAggregator(nn.Module):
    """
    Aggregates (B, N, C) -> (B, N, D) via MLP-Mixer.
    """
    def __init__(
        self,
        in_dim: int,          # C
        out_dim: int,         # D
        n_tokens: int,        # fixed N you will use
        d_model: int = 256,   # internal width
        num_blocks: int = 4,
        token_mlp_dim: int = 128,   # hidden size of token MLP
        channel_mlp_dim: int = 1024,# hidden size of channel MLP
        dropout: float = 0.0
    ):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, pad_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        x: (B, N, C)
        pad_mask: optional (B, N) with True on PAD positions
        """
        B, N, _ = x.shape
        assert N == self.n_tokens, f"Expected N={self.n_tokens}, got N={N}. (MLP-Mixer token MLP is length-tied.)"

        h = self.input_proj(x)  # (B, N, d_model)
        for blk in self.blocks:
            h = blk(h, pad_mask=pad_mask)  # (B, N, d_model)
        y = self.output_proj(h)            # (B, N, D)
        return y
...
class Blipv2(nn.Module):
    def __init__(self,):
        ...
        self.bbox_regression_head = nn.Sequential(nn.Linear(256, 128), nn.ReLU(), nn.Linear(128,4))
        self.classification_head = nn.Sequential(nn.Linear(256, 128), nn.ReLU(), nn.Linear(128, 80))

        self.bbox_regression_head2 = nn.Sequential(nn.Linear(256, 128), nn.ReLU(), nn.Linear(128,4))
    ...
    def forward(self, key_frame, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None, training=True):
        ...
        if training:
            ...
            loss = 0*torch.mean(loss_bbox) + 0*torch.mean(loss_cls) + 2*torch.mean(loss_bbox2) + torch.mean(loss_cls2) #+ mmd_loss  #+ self.center_pull_loss(output, ann.float().cuda())
        ...
        return loss, cls_results2, residual_boxes2, ann#, output
