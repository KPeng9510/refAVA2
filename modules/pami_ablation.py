...
class Blipv2(nn.Module):
    ...
    def forward(self, key_frame, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None, training=True):
        ...
        #print(bboxes.shape)
        output = self.cross_attention_box(bboxes, features_image_proj.mean(2)).mean(1)  + self.cross_attention_sem(features_text, features_image_proj.mean(2)).mean(1) +  self.cross_attention(agg_trajs, features_image_proj.mean(2)).mean(1)#+ self.proj_feature_text(features_text) # [B, T, D] 
        output_2 = self.cross_attention_box2(bboxes, features_image_proj.mean(1)).mean(1)  + self.cross_attention_sem2(features_text, features_image_proj.mean(1)).mean(1) + self.cross_attention_2(agg_trajs, features_image_proj.mean(1)).mean(1)
        #+ self.proj_feature_text_2(features_text) 
