...
target_modules=["attention.query", "attention.key", "attention.value", "attention.dense"]
config = LoraConfig(r=4, lora_alpha=32, lora_dropout=0.05, bias="none", target_modules=target_modules)
...
class AgentAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
        ...
        self.dwt_proj = nn.Linear(dim, dim)
        self.dwt_proj2 = nn.Linear(dim, dim)
        ...
        self.softmax = nn.Softmax(dim=-1)
        self.gate_net = GatingNetwork(input_size=dim,num_experts=3)
        self.gate_net_att = GatingNetwork(input_size=dim,num_experts=3)
    ...
    def forward(self, x, attn_1=None, attn_2=None, agent_tk1=None, agent_tk2=None):
        ...
        p_x = self.dwt_proj(x)
        #print(p_x.shape)
        dwt_x_h, dwt_x_l = self.dwt(p_x.permute(0,2,1))#.permute(0,2,1) #sequence Length b, n//2, c
        #print(dwt_x_l[0].shape)
        #print(dwt_x_l[1].shape)
        #print(dwt_x_l[2].shape)
        dwt_x = [dwt_x_h, dwt_x_l[0], dwt_x_l[1], dwt_x_l[2]]
        dwt_x = torch.cat(dwt_x, -1).permute(0,2,1)
        l_h, l_l_0, l_l_1, l_l_2 = dwt_x_h.shape[-1], dwt_x_l[0].shape[-1],  dwt_x_l[1].shape[-1],  dwt_x_l[2].shape[-1]

        dwt_x = self.dwt_proj2(dwt_x)
        n_dwt = dwt_x.shape[1]
        q = self.q(x)
        kv = self.kv(dwt_x).contiguous().view(b, -1, 2, c).contiguous().permute(2, 0, 1, 3).contiguous()
        ...
        k = k.reshape(b, n_dwt, num_heads, head_dim).contiguous().permute(0, 2, 1, 3).contiguous()
        v = v.reshape(b, n_dwt, num_heads, head_dim).contiguous().permute(0, 2, 1, 3).contiguous()


        agent_tokens = agent_tokens.mean(1).contiguous().reshape(b, self.agent_num, num_heads, head_dim).permute(0, 2, 1, 3).contiguous()

        weights_attn = self.gate_net_att(agent_tokens.mean(1).mean(1))
        agent_attn = self.softmax((agent_tokens * self.scale) @ k.contiguous().transpose(-2, -1).contiguous())
        #print(agent_attn.shape)
        agent_rep = agent_attn
        _,s_1,s_2,s_3 = agent_attn.shape


        if attn_1 != None and attn_2 != None:

            agent_attn = weights_attn[:,0].unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat(1, s_1, s_2, s_3)*agent_attn * torch.nn.functional.softmax(attn_1 + attn_2, -1) +  weights_attn[:,1].unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat(1, s_1, s_2, s_3)*agent_attn * torch.nn.functional.softmax(attn_1 + attn_2, 1) +  weights_attn[:,2].unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat(1, s_1, s_2, s_3)*agent_attn
        agent_attn = self.attn_drop(agent_attn)
        agent_v = agent_attn @ v
        _,s_1,s_2,s_3 = agent_tokens.shape
        weights = self.gate_net(agent_tokens.mean(1).mean(1))
        if agent_tk1 != None and agent_tk2 != None:
            agent_tokens = weights[:,0].unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat(1, s_1, s_2, s_3) * agent_tokens * torch.nn.functional.softmax(agent_tk1 + agent_tk2, -1) + weights[:,1].unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat(1, s_1, s_2, s_3) * agent_tokens * torch.nn.functional.softmax(agent_tk1 + agent_tk2, 1) + weights[:,2].unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat(1, s_1, s_2, s_3)* agent_tokens
        q_attn = self.softmax((q * self.scale) @ agent_tokens.contiguous().transpose(-2, -1))
        
        q_attn = self.attn_drop(q_attn)
        x = q_attn @ agent_v
        x = x.contiguous().transpose(1, 2).contiguous().view(b, -1, c)

        x = x #+ self.dwc(v_).contiguous().permute(0, 2, 3, 1).contiguous().reshape(b, n - 1, c).contiguous()

        dwt_p_x_h = dwt_x[:, :l_h, :].permute(0,2,1)
        dwt_p_x_l = [dwt_x[:,l_h:l_l_0, :].permute(0,2,1), dwt_x[:,l_l_0:l_l_1, :].permute(0,2,1), dwt_x[:,l_l_1:, :].permute(0,2,1)]
        
        #dwt_p_x_l = torch.stack(dwt_p_x_l,-1)
        #print(dwt_x[:,l_h:l_l_0, :].permute(0,2,1).shape)
        x = self.proj(x) + self.idwt((dwt_x_h, dwt_x_l)).permute(0,2,1)
        x = self.proj_drop(x)
...
class Blipv2(nn.Module):
    def __init__(self,):
        ...
        self.model =  wrap_model(self.model, target_modules, slowfastlora_config)
        set_no_grad(self.model)
        self.proj = nn.Linear(772,768)

        self.agent_temporal_slow = AgentBlock(dim=768, num_heads=1, window=8) #AgentAttention(dim=768)
        self.agent_temporal_fast = AgentBlock(dim=768, num_heads=1, window=8)
        #self.dino_processor = AutoProcessor.from_pretrained(model_id)
        device = "cuda"
        #self.grounding_dino =  AutoModelForZeroShotObjectDetection.from_pretrained(model_id).to(device)
        ...
        self.bbox_regressio_head_fast = nn.Sequential(nn.Linear(768, 512), nn.ReLU(), nn.Linear(512,4))
        self.classification_head_fast = nn.Sequential(nn.Linear(768, 512), nn.Dropout(0.2), nn.ReLU(), nn.Linear(512, 80))
    ...
    def compute_bbox_loss(self, target_bboxes, pred_bboxes):
    ...
    def forward(self, key_frame, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None,inputs=None, training=True):
        #print('test')
        ...
        with torch.no_grad():
            ...
            features_image = self.model.extract_features(sample, mode="image") # batch_size, 8, 32, 768 self.model.get_qformer_features(video.half().cuda(), return_dict=True)
            features_text = self.model.extract_features(sample, mode="text") # batch_size, 12, 768 self.model.get_text_features(inputs_text["input_ids"].cuda())#
        #features_text = self.text_proj(features_text['logits'])
        #print(bboxes.shape)
        boxes = self.proj_box(bboxes.contiguous().permute(0,2,1).cuda()).contiguous().permute(0,2,1)
        #print(boxes.shape)
        #boxes = self.proj(boxes)
        features_text = features_text.text_embeds.contiguous().view(b, ts,-1, 768).mean(1).mean(1).unsqueeze(1).repeat(1,32,1)
        ...
        #features_text = features_text.contiguous().view(b_real, ts,-1, 768).mean(1)
        ...
        features_slow = features_image[:b_real]
        #features_fast = features_image[b_real:]

        features_image_slow = features_slow 
        #features_image_fast = features_fast
        #print(features_image_slow.shape)
        features_image_slow,_, _ = self.agent_temporal_slow(features_image_slow.mean(1), attn_sem, attn_box, a_t_s, a_t_b)

        #features_image_fast,_, _ = self.agent_temporal_fast(features_image_fast.mean(1), attn_sem, attn_box, a_t_s, a_t_b)

        logits = boxes.mean(1) + features_text.mean(1) + features_image_slow.mean(1)
        #logits_fast = 0.1*boxes.mean(1) + 0.1*features_text.mean(1) + 0.8*features_image_fast.mean(1)
        #logits = logits + logits_fast


        logits_b = boxes.mean(1) + features_text.mean(1) + features_image_slow.mean(1)
        #logits_fast_b = 0.6*boxes.mean(1) + 0.2*features_text.mean(1) + 0.2*features_image_fast.mean(1)
        #logits_b = logits_b + logits_fast_b

        bbox_prediction = self.bbox_regressio_head(logits_b)
        ...
        if training:
            ...
            #ann = gather(ann)
            loss_cls = self.bceloss(predictions, ann.float().cuda()) #+  self.bceloss(predictions_fast, ann.float().cuda())
            #loss_cls = loss_cls
            batch_size = bbox_prediction.shape[0]
            loss_bbox = self.mseloss(bbox_prediction.reshape(batch_size, 4), bbox.float().cuda()) # + self.mseloss(bbox_prediction_fast.reshape(batch_size, 4), bbox.float().cuda())
            #loss_bbox_2 = self.compute_bbox_loss(bbox_prediction.reshape(batch_size, 4), bbox.float().cuda()) + self.compute_bbox_loss(bbox_prediction_fast.reshape(batch_size, 4), bbox.float().cuda())
