...
class Blipv2(nn.Module):
    def __init__(self,):
        ...
        self.bbox_regression_head = nn.Sequential(nn.Linear(768, 256), nn.ReLU(), nn.Linear(256,4), nn.Sigmoid())
        self.classification_head = nn.Sequential(nn.Linear(768, 512), nn.ReLU(), nn.Linear(512, 80))

        self.bbox_regression_head2 = nn.Sequential(nn.Linear(768, 128), nn.ReLU(), nn.Linear(128,4), nn.Sigmoid())
        self.classification_head2 = nn.Sequential(nn.Linear(768, 256), nn.ReLU(), nn.Linear(256, 80))




        self.mamba_fusion = nn.Sequential(nn.Linear(768, 256), Mamba(d_model=256), Mamba(d_model=256), nn.Linear(256, 768)) #Mamba(d_model=768)
        self.mamba_fusion_temp = nn.Sequential(nn.Linear(768, 256), Mamba(d_model=256), Mamba(d_model=256), nn.Linear(256, 768)) #Mamba(d_model=768)
        self.mamba_image = nn.Sequential(nn.Linear(768, 256), Mamba(d_model=256), Mamba(d_model=256), nn.Linear(256, 768))
        ...
        self.cross_attention_box = CrossAttentionBlock(embed_dim=768, num_heads=8)
        ...
        self.cross_attention_box2 = CrossAttentionBlock(embed_dim=768, num_heads=8)
    ...
    def forward(self, key_frame, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None, training=True):
        ...
        features_image_bboxes = features_image.mean(2)
        ...
        cls_results2 = self.classification_head2(output)
        ...
        if training:
            ...
            #print(ann.shape)
            #ann = ann[:,0]
            loss_cls = focal_binary_cross_entropy_label_smooth(cls_results, ann.float().cuda()) + auc_rank_loss(cls_results, ann.float().cuda()) + irm_penalty(cls_results, ann.float().cuda())
            loss_bbox = self.mseloss(residual_boxes, bbox.float().cuda()) + iou_loss(residual_boxes, bbox.float().cuda())

            loss_cls2 = focal_binary_cross_entropy_label_smooth(cls_results2, ann.float().cuda()) + auc_rank_loss(cls_results2, ann.float().cuda()) + irm_penalty(cls_results2, ann.float().cuda())
            loss_bbox2 = self.mseloss(residual_boxes2, bbox.float().cuda()) + iou_loss(residual_boxes2, bbox.float().cuda())
            


            loss = 2*torch.mean(loss_bbox) + torch.mean(loss_cls) + 2*torch.mean(loss_bbox2) + torch.mean(loss_cls2)  #+ self.center_pull_loss(output, ann.float().cuda())
