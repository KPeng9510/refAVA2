...
class Blipv2(nn.Module):
    def __init__(self,):
        ...
        self.detection_model =  torchvision.models.detection.retinanet_resnet50_fpn(weights=torchvision.models.detection.RetinaNet_ResNet50_FPN_Weights.DEFAULT)#detr_resnet50(pretrained=True).eval()#torch.hub.load('facebookresearch/detr', 'detr_resnet50', pretrained=True, force_reload=True)
    ...
    def forward(self, key_frame, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None, training=True):
        ...
        b, pair, bs, ts, channel, h1, w1 = video.shape


        tg = []
        for i in range(key_frame.shape[0]):
            target_dict = {}
            target_dict["boxes"] = bbox[i].unsqueeze(0)
            target_dict["labels"] = torch.ones([1]).cuda().unsqueeze(0)
            tg.append(target_dict)

        self.detection_model.training = False
        h,w = key_frame.shape[-2], key_frame.shape[-1]
        detect_results = self.detection_model(key_frame.squeeze())
        #print(detect_results)
        preds = []
        boxes = []
        for det in detect_results:
            #preds.append(det["labels"])
            pr = det["labels"]
            bx = det["boxes"]
            if bx.shape[0] == 0:
                boxes.append(torch.zeros([100,4]).cuda().unsqueeze(0))
                preds.append(torch.Tensor([90]*100).cuda())                
            if bx.shape[0] >= 100:
                bx = bx[:100]
                boxes.append(torch.stack([bx[...,0]/h,bx[...,1]/w,bx[...,2]/h,bx[...,3]/w],-1))
                preds.append(pr[:100])
            if bx.shape[0] < 100 and bx.shape[0] !=0:
                all_boxes = []
                all_preds = []
                all_boxes.append(torch.stack([bx[...,0]/h,bx[...,1]/w,bx[...,2]/h,bx[...,3]/w],-1))
                all_preds.append(pr)
                for k in range(100-bx.shape[0]):
                    all_boxes.append(torch.zeros_like(bx[0]).cuda().unsqueeze(0))
                    all_preds.append(torch.Tensor([90]).cuda())
                preds.append(torch.cat(all_preds, 0))
                boxes.append(torch.cat(all_boxes, 0))

        preds = torch.stack(preds,0)
        try:
            boxes = torch.stack(boxes,0)
        except:
            boxes = torch.stack([bd.squeeze() for bd in boxes],0)
        preds = preds -1 # B,Q,N
        #boxes = detect_results["boxes"] # B,Q,N
        B = preds.shape[0]
        #print(boxes.shape)
        #print(preds.shape)
        categories = preds.flatten().long()
        ...
        video = video.contiguous().view(b * pair * bs* ts, channel, h1, w1)
        video_frame = bs * ts
        #with torch.no_grad():
        #print(b)
        #print(input_ids)
