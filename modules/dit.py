...
class MFDiT(nn.Module):
    def __init__(
        ...
        self.x_embedder = nn.Linear(self.num_classes, dim)  #LabelEmbedder(self.num_classes, dim) 
        self.t_embedder = TimestepEmbedder(dim)
        self.r_embedder = TimestepEmbedder(dim)
    
        self.use_cond = num_classes is not None
        self.y_embedder = LabelEmbedder(self.input_dimension, dim)

        self.blocks = nn.ModuleList([
            DiTBlock(dim, num_heads, mlp_ratio) for _ in range(depth)
        ])
        self.final_layer = FinalLayer(dim, 1, self.num_classes)
    ...
    def forward(self, x, t, r, y=None):
        ...
        x = self.x_embedder(x.float()).unsqueeze(1) #+ self.pos_embed  # (N, T, D), where T = H * W / patch_size ** 2
