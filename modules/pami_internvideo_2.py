...
from sam2_re.sam2_image_predictor import SAM2ImagePredictor  # official predictor
import torch
...
def to_uint8_torch(x: torch.Tensor) -> torch.Tensor:
    # x can be (H,W), (H,W,3), (C,H,W), or (B,C,H,W) with floats in [0,1]
...
def resize_spatial(x):
    ...
    return x
def sam2_features_from_video_tensor(
    video_btc_hw: torch.Tensor,
    model,
    device: Union[str, torch.device] = "cuda",
    batch_size: int = 8,
    return_pooled: Literal["none", "avg", "max"] = "avg",
) -> Tuple[torch.Tensor, Union[torch.Tensor, None]]:
    """
    Compute SAM-2 image-encoder embeddings for every frame.
    Args:
        video_btc_hw: (B, T, C, H, W) torch tensor.
        model_id: HF repo id for SAM2 weights, e.g., "facebook/sam2-hiera-large".
        device: "cuda" or "cpu".
        batch_size: number of frames to encode at once.
        return_pooled: if "avg" or "max", also return (B,T,C_emb) pooled vectors.

    Returns:
        feats_5d:  (B, T, C_emb, H_emb, W_emb) torch.float32 on CPU
        pooled:    (B, T, C_emb) or None
    """
    assert video_btc_hw.ndim == 5, "expected (B,T,C,H,W)"
    B, T, C, H, W = video_btc_hw.shape
    batch_size = B*T
    # 1) Build predictor and move model to device
    predictor = model #
    predictor.model.cuda()  # predictor wraps the model internally. :contentReference[oaicite:2]{index=2}

    # 2) Flatten frames -> list of RGB uint8 images (H,W,C)
    frames: List[np.ndarray] = []
    for b in range(B):
        for t in range(T):
            frames.append(video_btc_hw[b, t])

    # 3) Encode in mini-batches using set_image_batch(...)
    #    Then read back embeddings via get_image_embedding() (which returns cached embeddings). :contentReference[oaicite:3]{index=3}
    all_embs: List[torch.Tensor] = []
    i = 0
    while i < len(frames):
        batch_imgs = frames[i : i + batch_size]
        with torch.autocast(str(device), dtype=torch.bfloat16 if "cuda" in str(device) else torch.float32):
            #print(batch_imgs[0].shape)
            predictor.set_image_batch(batch_imgs)   # computes & caches per-image embeddings
        # get_image_embedding() returns the cached embeddings; in batched mode it's a list or stack.
        embs = predictor.get_image_embedding()      # typically [N, C_emb, H_emb, W_emb] or list of 1xC_embxH_embxW_emb
        if isinstance(embs, list):
            embs = torch.cat(embs, dim=0)          # (N, C_emb, H_emb, W_emb)
        # Move to CPU fp32 for portability
        all_embs.append(embs)
        # Free predictor state before next batch (avoids growing caches)
        if hasattr(predictor, "reset_predictor"):
            predictor.reset_predictor()
        i += batch_size

    feats = torch.cat(all_embs, dim=0)              # (B*T, C_emb, H_emb, W_emb)
    #print(feats.shape)
    #import os
    #os.exit()
    # 4) Reshape back to (B, T, C_emb, H_emb, W_emb)
    C_emb, H_emb, W_emb = feats.shape[1:]
    feats_5d = feats.view(B, T, C_emb, H_emb, W_emb)

    # 5) Optional temporal/spatial pooling per frame
    pooled = None
    if return_pooled == "avg":
        pooled = feats_5d.mean(dim=(3, 4))          # (B, T, C_emb)
    elif return_pooled == "max":
        pooled = feats_5d.view(B, T, C_emb, -1).max(dim=-1).values  # (B, T, C_emb)

    return feats_5d, pooled
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, x_text, x_vis, vis_pad_mask=None):
        """
        x_text: [B, M, C]  (queries)
        x_vis:  [B, T, C]  (keys/values)
        vis_pad_mask: [B, T] -> True for pads (PyTorch convention)
        """
        # Cross-attn: text queries attend to vision tokens
        attn_out, _ = self.cross_attn(
            query=x_text,
            key=x_vis,
            value=x_vis,
            key_padding_mask=vis_pad_mask
        )
        x_text = self.norm1(x_text + attn_out)
        # FFN
        ffn_out = self.ffn(x_text)
        x_text = self.norm2(x_text + ffn_out)
        return x_text
...
class SelfAttnBlock(nn.Module):
    ...
    def forward(self, x, pad_mask=None, attn_mask=None):
        sa_out, _ = self.self_attn(x, x, x, key_padding_mask=pad_mask, attn_mask=attn_mask)
...
class MultiModalFusion(nn.Module):
    """
    Stack L blocks. Each layer can:
      1) (optional) self-attend text
      2) (optional) self-attend vision
      3) cross-attend text <- vision
    Output: fused text tokens [B, M, C]
    """
    def __init__(
        self,
        embed_dim=768,
        ...
        dropout=0.1,
        self_attn_text=True,
        self_attn_vision=False,
        share_params=False,   # set True to ALBERT-style parameter sharing
    ):
        super().__init__()
        self.self_attn_text = self_attn_text
        self.self_attn_vision = self_attn_vision

        layer = nn.ModuleDict({
            "sa_txt": SelfAttnBlock(embed_dim, num_heads, dropout) if self_attn_text else nn.Identity(),
            "sa_vis": SelfAttnBlock(embed_dim, num_heads, dropout) if self_attn_vision else nn.Identity(),
            "xa":     CrossAttentionBlock(embed_dim, num_heads, dropout),
        })

        if share_params:
            # Reuse the same layer L times
    ...
    def forward(
        ...
        x_text,                  # [B, M, C]
        x_vis,                   # [B, T, C]
        text_pad_mask=None,      # [B, M] True for pads
        vis_pad_mask=None,       # [B, T] True for pads
        text_attn_mask=None,     # [M, M] or [B*num_heads, M, M] if causal/etc.
        vis_attn_mask=None       # same idea for vision self-attn
    ):
        for layer in self.layers:
            # 1) optional self-attention on text
            if self.self_attn_text:
                x_text = layer["sa_txt"](x_text, pad_mask=text_pad_mask, attn_mask=text_attn_mask)
            # 2) optional self-attention on vision
            if self.self_attn_vision:
                x_vis = layer["sa_vis"](x_vis, pad_mask=vis_pad_mask, attn_mask=vis_attn_mask)
            # 3) cross-attention: text queries attend to vision
            x_text = layer["xa"](x_text, x_vis, vis_pad_mask)
        return x_text  # fused text-side representation
...
class Blipv2(nn.Module):
    def __init__(self,):
        ...
        #self.model = AutoModel.from_pretrained("OpenGVLab/InternVideo2-Stage2_6B", trust_remote_code=True).eval()
        model_id = "facebook/sam2-hiera-large"
        #self.sam2_model = SAM2ImagePredictor.from_pretrained(model_id)

        '''for block in self.model.visual_encoder.blocks:
        ...
        self.bbox_regression_head = nn.Sequential(nn.Linear(768,128), nn.ReLU(), nn.Linear(128, 4))
        #self.bbox_regressio_head = nn.Sequential(nn.Linear(768, 256), nn.ReLU(), nn.Linear(256,4), nn.Sigmoid())
        self.classification_head = nn.Sequential(nn.Linear(768, 128), nn.ReLU(), nn.Linear(128, 80))
        self.mamba_fusion =  Mamba(d_model=768)
        self.mamba_fusion_temp = Mamba(d_model=768)
        self.mamba_image = Mamba(d_model=768)
        config = AutoConfig.from_pretrained("OpenGVLab/VideoMAEv2-Base", trust_remote_code=True)
        self.video_model = AutoModel.from_pretrained('OpenGVLab/VideoMAEv2-Base', config=config, trust_remote_code=True)
        self.video_model.fc_norm = None
        self.bceloss = torch.nn.BCEWithLogitsLoss()
        self.mseloss = torch.nn.MSELoss()
        self.cross_attention = CrossAttentionBlock(embed_dim=768, num_heads=8)
        self.cross_attention_2 = CrossAttentionBlock(embed_dim=768, num_heads=8)

        self.cross_attention_sem = CrossAttentionBlock(embed_dim=768, num_heads=8)
        self.cross_attention_sem2 = CrossAttentionBlock(embed_dim=768, num_heads=8)
    ...
    def roberta_embed(
        ...
        """
        Returns a [num_sentences, hidden_size] tensor of embeddings.
        pooling='cls'  -> use the first token (<s>) representation
        pooling='mean' -> mean-pool token embeddings with attention mask
        """
        ...
        # Simple batching to avoid OOM on long lists
        for i in range(0, len(sentences), batch_size):
            ...
            out = self.text_model(**enc)  # last_hidden_state: [B, T, H], pooler_output is None for RoBERTa

            if pooling == "cls":
                # RoBERTa's first token <s> acts like [CLS]
                batch_emb = out.last_hidden_state[:, :, :]  # [B, H]
            elif pooling == "mean":
    ...
    def forward(self, key_frame, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None, training=True):
        ...
        video2 = video.contiguous().view(b * pair * bs, ts, channel, h, w).permute(0,2,1,3,4)
        #video = video.contiguous().view(b * pair * bs, ts, channel, h, w)
        with torch.no_grad():
            video_feat = self.video_model.extract_features(video2.cuda())
            text_feat = self.roberta_embed(input_ids)
        fused_feat = self.multi_modal_fusion(text_feat, video_feat)
        #print(fused_feat.shape)
        #print(video_feat.shape)
        #print(text_feat.shape)
        #feat,pooled = sam2_features_from_video_tensor(video.to(torch.bfloat16).cuda(), self.sam2_model)
        #print(feat.shape)
        #video2 = video2.contiguous().view(b * pair * bs * ts, channel, 224, 224)
        ...
        '''with torch.no_grad():
            ...
            sample = {"image": video2.half().cuda(), "text_input": text}
            ...
            features_image = features_image.image_embeds.contiguous().view(b, ts, -1, 768).mean(2)
        bboxes,logits = predict(self.grounding_dino,video2,  text, BOX_TRESHOLD,TEXT_TRESHOLD)'''
        #print(logits.shape)
        #print(features_text.mean(1).shape)
        pooled = fused_feat.mean(1)#video_feat +  text_feat
        #print('test')
        '''print(feat.shape)
        print(pooled.shape)
        import os
        ...
        output = self.cross_attention(agg_trajs, features_image).mean(1) + self.cross_attention_sem(features_text, features_image).mean(1) #+ self.proj_feature_text(features_text) # [B, T, D]
        output_2 = self.cross_attention_2(agg_trajs, features_image).mean(1) + self.cross_attention_sem2(features_text, features_image).mean(1) #+ self.proj_feature_text_2(features_text)'''


        residual_boxes = self.bbox_regression_head(pooled)#pred_boxes.cuda()
        cls_results = self.classification_head(pooled)
        ...
        if training:
            ...
            #ann = gather(ann)
            loss_cls = focal_binary_cross_entropy(cls_results, ann.float().cuda()) + auc_rank_loss(cls_results, ann.float().cuda())
            ...
            loss = 0.0
        return loss, cls_results, residual_boxes, ann
