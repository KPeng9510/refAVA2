...
from modules.mean_flow import MeanFlow
...
class Blipv2(nn.Module):
    def __init__(self,):
        ...
        self.classification_hed = nn.Sequential(nn.Linear(768, 512), nn.Dropout(0.2), nn.ReLU(), nn.Linear(512, 256), nn.ReLU(), nn.Linear(256, 80))
        ...
        self.meanflow_cls = MeanFlow(channels=4,
                            image_size=32,
                            num_classes=80,
        ...
        self.meanflow_bbox = MeanFlow(channels=4,
                            image_size=32,
                            num_classes=4,
                            normalizer=['mean_std', 0.0, 1/latent_factor],
                            flow_ratio=0.50,
    ...
    def forward(self, key_frame, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None, training=True):
        ...
        features_image,_, _ = self.agent_temporal(features_image.mean(1), attn_sem, attn_box, a_t_s, a_t_b) # dimension768
        conditions = bboxes.mean(1) + features_text.mean(1) + features_image.mean(1) # dimension 768
        
        if training:
            loss_cls, mse_val_cls = self.meanflow_cls.loss(self.model_cls, ann, conditions)
            loss_bbox, mse_val_bbox = self.meanflow_bbox.loss(self.model_box, bbox,  conditions)
            total_loss = loss_cls + mse_val_cls + loss_bbox + mse_val_bbox
            return total_loss
        else:
            model_module_cls = self.model_cls.module if hasattr(self.model_cls, 'module') else self.model_cls
            z_cls = self.meanflow_cls.sample_each_class(model_module_cls,
                                            n_per_class=1,
                                            classes=conditions)            
    

            model_module_bbox = self.model_box.module if hasattr(self.model_box, 'module') else self.model_box
            z_bbox = self.meanflow_bbox.sample_each_class(model_module_bbox,
                                            n_per_class=1,
                                            classes=conditions)            
            #print(z_cls.shape)
            #print(z_bbox.shape)
            return z_cls.squeeze(), z_bbox.squeeze(), ann
