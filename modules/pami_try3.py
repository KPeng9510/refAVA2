...
class CrossAttentionBlock_multi_query(nn.Module):
    def __init__(self, embed_dim, num_heads=4, dropout=0.0, num_queries: int = 6):
        """
        num_queries: if > 0, create learnable queries of shape [num_queries, D]
        """
        super().__init__()
        self.attn_q1 = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, batch_first=True)
        #self.attn_q2 = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, batch_first=True)
        #self.attn_q3 = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, batch_first=True)
        self.norm1 = nn.LayerNorm(embed_dim)
    ...
    def forward(self, context, q1=None, q2=None, q3=None, append_learned: bool = False, return_split: bool = False):
        """
        context: [B, T_kv, D]  (e.g., features_image)
        query:   [B, T_q,  D]  (optional external queries)
        ...
        q1_fft = torch.abs(torch.fft.rfft(q1, dim=1))
        #print(q1_fft)
        q1 = torch.cat([q1, q1_fft], dim=1)

        q2_fft = torch.abs(torch.fft.rfft(q2, dim=1))
        q2 = torch.cat([q2, q2_fft], dim=1)

        q3_fft = torch.abs(torch.fft.rfft(q3, dim=1))
        q3 = torch.cat([q3, q3_fft], dim=1)
        ...
        if q1 is None:
            ...
            q1 = torch.cat([q1,q2,q3, learned_q], dim=1)      # [B, T_q + NQ, D]
            #q2 = torch.cat([q2, learned_q], dim=1)
            #q3 = torch.cat([q3, learned_q], dim=1)

            split_sizes1 = (q1.size(1) + q2.size(1) + q3.size(1), learned_q.size(1))
        else:
            q1 = q1
            split_sizes = None

        # Cross-attention: queries attend to the context (keys/values)
        context_fft = torch.abs(torch.fft.rfft(context, dim=1))
        context = torch.cat([context, context_fft], dim=1)


        attn_output_q1, _ = self.attn_q1(query=q1, key=context, value=context)
        #attn_output_q2, _ = self.attn_q2(query=q2, key=context, value=context)
        #attn_output_q3, _ = self.attn_q3(query=q3, key=context, value=context)
        # Add & norm
        x1 = self.norm1(q1 + attn_output_q1)
        #x2 = self.norm1(q2 + attn_output_q2)
        #x3 = self.norm1(q3 + attn_output_q3)

        # Feedforward with residual
        x = x1#torch.cat([x1, x2, x3], dim=1)
...
class GatedMultiQueryCrossAttentionWithLearnedQs(nn.Module):
    """
    Cross-attention where the effective query is a learned mixture of three inputs
    with possibly different lengths, augmented with N learnable query tokens.
    ...
    Returns (T_ref depends on ref_stream):
        x_tokens:        [B, T_ref, D]  # outputs at reference query positions
        x_learned:       [B, Nq,   D]   # outputs for learned queries
        learned_summary: [B, D]         # mean of x_learned (pooled rep; falls back to x_tokens mean if Nq=0)
        gates:           [B, T_ref, 3]  # gate probs over (q1,q2,q3) at each ref position
    """
    def __init__(
        self,
        embed_dim: int,
        ...
        per_token: bool = True,
        use_context_summary: bool = True,
        hard_top1: bool = True,
        gumbel_tau: float = 1.0,
        n_learned_queries: int = 10,          # >0 to enable learned queries
        learned_q_dropout: float = 0.0,      # dropout on learned queries (token-wise)
        fuse_learned_back: bool = True,     # if True, add learned summary back to x_tokens
        fuse_scale: float = 1.0,
        ref_stream: str = "longest",         # 'q1' | 'q2' | 'q3' | 'longest'
    ):
        super().__init__()
        self.per_token = per_token
        self.use_context_summary = use_context_summary
        self.hard_top1 = hard_top1
        self.gumbel_tau = gumbel_tau
        self.fuse_learned_back = fuse_learned_back
        self.fuse_scale = fuse_scale
        self.ref_stream = ref_stream

        # --- Attentions ---
        # 1) Align other query streams to the reference stream positions
        self.align_attn = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, batch_first=True)
        # 2) Cross-attend the mixed queries over the context
        self.attn_ctx   = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, batch_first=True)

        # --- Norms/FFN for token outputs ---
        self.norm1_tokens = nn.LayerNorm(embed_dim)
        self.ff = nn.Sequential(
            nn.Linear(embed_dim, embed_dim * 2),
            nn.GELU(),
            nn.Linear(embed_dim * 2, embed_dim),
        ...
        self.norm2_tokens = nn.LayerNorm(embed_dim)

        # --- Gating network ---
        in_dim = embed_dim * 3 + (embed_dim if use_context_summary else 0)
        gh = gating_hidden or max(128, embed_dim // 2)
        ...
        self.n_learned = int(n_learned_queries)
        if self.n_learned > 0:
            self.learned_q = nn.Parameter(
                torch.randn(1, self.n_learned, embed_dim) * (1.0 / embed_dim ** 0.5)
            )  # [1, Nq, D], broadcast over batch
            self.learned_q_drop = nn.Dropout(learned_q_dropout)
            self.norm1_learned = nn.LayerNorm(embed_dim)
            self.norm2_learned = nn.LayerNorm(embed_dim)

    # ---------- helpers ----------
    def _pick_ref(self, q1, q2, q3):
        lens = [q1.size(1), q2.size(1), q3.size(1)]
        if self.ref_stream == "q1":
            idx = 0
        elif self.ref_stream == "q2":
            idx = 1
        elif self.ref_stream == "q3":
            idx = 2
        else:
            # default: use the longest stream for best coverage
            idx = int(torch.tensor(lens).argmax().item())
        ref = [q1, q2, q3][idx]
        return ref, idx  # ref: [B, T_ref, D], idx in {0,1,2}

    def _context_summary(self, context: torch.Tensor) -> torch.Tensor:
        return context.mean(dim=1, keepdim=True)  # [B,1,D]

    def _align_to_ref(self, q_ref: torch.Tensor, qi: torch.Tensor):
        """
        Align arbitrary-length stream qi to reference positions of q_ref via attention:
        aligned = Attn(Q=q_ref, K=qi, V=qi) -> [B, T_ref, D]
        """
        aligned, _ = self.align_attn(query=q_ref, key=qi, value=qi)
        return aligned

    def _compute_gates(self, a1, a2, a3, context, per_token=True):
        """
        a1/a2/a3: aligned features, each [B, T_ref, D]
        returns gates:[B,T_ref,3], logits:[B,T_ref,3]
        """
        B, Tref, D = a1.shape
        csum = self._context_summary(context) if self.use_context_summary else None

        if per_token:
            if csum is not None:
                csum = csum.expand(B, Tref, D)
                gate_in = torch.cat([a1, a2, a3, csum], dim=-1)   # [B,Tref,4D]
            else:
                gate_in = torch.cat([a1, a2, a3], dim=-1)         # [B,Tref,3D]
            logits = self.gate_mlp(gate_in)                        # [B,Tref,3]
        else:
            parts = [a1.mean(1, keepdim=True), a2.mean(1, keepdim=True), a3.mean(1, keepdim=True)]
            if csum is not None:
                parts.append(csum)
            gate_in = torch.cat(parts, dim=-1).squeeze(1)          # [B, *D]
            logits = self.gate_mlp(gate_in).unsqueeze(1).expand(B, Tref, 3)

        if self.hard_top1:
            soft = F.softmax(logits, dim=-1)
            hard = F.gumbel_softmax(logits, tau=self.gumbel_tau, hard=True, dim=-1)
            gates = hard + soft - soft.detach()  # straight-through for top-1
        else:
    ...
    # ---------- forward ----------
    def forward(self, q1: torch.Tensor, q2: torch.Tensor, q3: torch.Tensor, context: torch.Tensor):
        """
        q1: [B,T1,D], q2:[B,T2,D], q3:[B,T3,D], context:[B,Tk,D]
        """
        B, _, D = q1.shape
        # 1) choose the reference time axis
        q_ref, ref_idx = self._pick_ref(q1, q2, q3)                # [B,T_ref,D]
        T_ref = q_ref.size(1)

        # 2) align all streams to reference positions
        aligned = [None, None, None]
        streams = [q1, q2, q3]
        for i, qi in enumerate(streams):
            if i == ref_idx:
                aligned[i] = q_ref                                  # identity for the ref stream
            else:
                aligned[i] = self._align_to_ref(q_ref, qi)          # [B,T_ref,D]
        a1, a2, a3 = aligned

        # 3) gating over aligned triplet
        gates, _ = self._compute_gates(a1, a2, a3, context, per_token=self.per_token)  # [B,T_ref,3]
        a_stack = torch.stack([a1, a2, a3], dim=2)                  # [B,T_ref,3,D]
        q_mix = (a_stack * gates.unsqueeze(-1)).sum(dim=2)          # [B,T_ref,D]

        # 4) append learned queries (if any)
        queries_list = [q_mix]
        if self.n_learned > 0:
            learned_q = self.learned_q.expand(B, -1, -1)            # [B,Nq,D]
            learned_q = self.learned_q_drop(learned_q)
            queries_list.append(learned_q)
        Q = torch.cat(queries_list, dim=1)                          # [B, T_ref(+Nq), D]

        # 5) cross-attention over the provided context

        #print(Q.shape)
        #print(context.shape)
        #print()

        attn_out, _ = self.attn_ctx(query=Q, key=context, value=context)  # [B, T_ref(+Nq), D]

        # 6) split token/learned segments
        x_tokens = attn_out[:, :T_ref, :]
        if self.n_learned > 0:
            x_learned = attn_out[:, T_ref:, :]
        else:
            x_learned = Q.new_zeros(B, 0, D)

        # 7) residual + norm (separate norms per segment)
        x_tokens = self.norm1_tokens(q_mix + x_tokens)
        x_tokens = self.norm2_tokens(x_tokens + self.ff(x_tokens))
        if self.n_learned > 0:
            x_learned = self.norm1_learned(queries_list[1] + x_learned)
            x_learned = self.norm2_learned(x_learned + self.ff(x_learned))

        # 8) optional fusion of learned summary back into tokens
        if self.n_learned > 0:
            learned_summary = x_learned.mean(dim=1)                 # [B,D]
            if self.fuse_learned_back:
                x_tokens = x_tokens + self.fuse_scale * learned_summary.unsqueeze(1)
        else:
            learned_summary = x_tokens.mean(dim=1)                  # fallback if no learned queries

        return x_tokens


class GatedMultiQueryCrossAttentionWithLearnedQs2(nn.Module):
    """
    Cross-attention where the effective query is a learned mixture of three inputs
    with possibly different lengths, augmented with N learnable query tokens.
    ...
    def __init__(
        ...
        hard_top1: bool = True,
        gumbel_tau: float = 1.0,
        n_learned_queries: int = 10,
        learned_q_dropout: float = 0.0,
        fuse_learned_back: bool = True,
        fuse_scale: float = 1.0,
        ref_stream: str = "longest",
        attn_dropout: float = 0.0,               # extra: explicit attn dropout
        residual_dropout: float = 0.0,           # extra: residual dropout
    ):
        super().__init__()
        self.per_token = per_token
        self.use_context_summary = use_context_summary
        ...
        # --- Attentions ---
        self.align_attn = nn.MultiheadAttention(embed_dim, num_heads, dropout=attn_dropout, batch_first=True)
        self.attn_ctx   = nn.MultiheadAttention(embed_dim, num_heads, dropout=attn_dropout, batch_first=True)
        ...
        self.n_learned = int(n_learned_queries)
        if self.n_learned > 0:
            # init scaled by 1/sqrt(D) helps
            self.learned_q = nn.Parameter(torch.randn(1, self.n_learned, embed_dim) / (embed_dim ** 0.5))
            self.learned_q_drop = nn.Dropout(learned_q_dropout)
            self.norm1_learned = nn.LayerNorm(embed_dim)
            self.norm2_learned = nn.LayerNorm(embed_dim)
    ...
    def _pick_ref(self, q1, q2, q3):
        ...
        if self.ref_stream == "q1":
            ...
            idx = int(torch.tensor(lens, device=q1.device).argmax().item())
        ref = [q1, q2, q3][idx]
        return ref, idx
    ...
    def _align_to_ref(self, q_ref: torch.Tensor, qi: torch.Tensor, key_padding_mask=None):
        # Align qi → ref positions
        aligned, _ = self.align_attn(query=q_ref, key=qi, value=qi, key_padding_mask=key_padding_mask)
        return aligned
    ...
    def _compute_gates(self, a1, a2, a3, context, per_token=True):
        ...
        if per_token:
            ...
            if csum is not None:
                parts.append(csum)
            gate_in = torch.cat(parts, dim=-1).squeeze(1)          # [B, *]
            logits = self.gate_mlp(gate_in).unsqueeze(1).expand(B, Tref, 3)

        if self.hard_top1:
            soft = F.softmax(logits, dim=-1)
            hard = F.gumbel_softmax(logits, tau=self.gumbel_tau, hard=True, dim=-1)
            gates = hard + soft - soft.detach()  # straight-through
        else:
            gates = F.softmax(logits, dim=-1)
    ...
    def forward(
        ...
        *,
        ...
        q2_pad_mask=None,
        q3_pad_mask=None,
        context_pad_mask=None,
    ):
        """
        q*: [B,Ti,D], context: [B,Tk,D]
        *_pad_mask: [B, Ti] True for PAD positions (nn.MultiheadAttention convention)
        """
        B, _, D = q1.shape

        # 1) reference axis
        q_ref, ref_idx = self._pick_ref(q1, q2, q3)
        T_ref = q_ref.size(1)

        # 2) align all streams to reference positions
        aligned = [None, None, None]
        streams = [q1, q2, q3]
        masks   = [q1_pad_mask, q2_pad_mask, q3_pad_mask]
        for i, (qi, mask_i) in enumerate(zip(streams, masks)):
            if i == ref_idx:
                aligned[i] = q_ref
            else:
                aligned[i] = self._align_to_ref(q_ref, qi, key_padding_mask=mask_i)
        a1, a2, a3 = aligned
        ...
        # 4) append learned queries
        queries = [q_mix]
        if self.n_learned > 0:
            learned_q = self.learned_q.expand(B, -1, -1)            # [B,Nq,D]
            learned_q = self.learned_q_drop(learned_q)
            queries.append(learned_q)
        Q = torch.cat(queries, dim=1)                               # [B, T_ref(+Nq), D]

        # 5) cross-attention over context
        attn_out, _ = self.attn_ctx(query=Q, key=context, value=context, key_padding_mask=context_pad_mask)

        # 6) split token/learned segments
        x_tokens  = attn_out[:, :T_ref, :]
        x_tokens  = self.norm1_tokens(self.resid_drop(x_tokens) + q_mix)
        x_tokens  = self.norm2_tokens(self.resid_drop(self.ff(x_tokens)) + x_tokens)

        if self.n_learned > 0:
            x_learned = attn_out[:, T_ref:, :]
            base_learned = queries[1]  # learned_q after dropout
            x_learned = self.norm1_learned(self.resid_drop(x_learned) + base_learned)
            x_learned = self.norm2_learned(self.resid_drop(self.ff(x_learned)) + x_learned)
            learned_summary = x_learned.mean(dim=1)                 # [B,D]
            if self.fuse_learned_back:
                x_tokens = x_tokens + self.fuse_scale * learned_summary.unsqueeze(1)
        else:
            x_learned = Q.new_zeros(B, 0, D)
            learned_summary = x_tokens.mean(dim=1)

        return x_learned #, learned_summary, gates
...
class Blipv2(nn.Module):
    def __init__(self,):
        ...
        self.mamba_fusion_temp = nn.Sequential(nn.Linear(768, 256), Mamba(d_model=256), Mamba(d_model=256))
        self.mamba_fusion = nn.Sequential(Mamba(d_model=768))
        ...
        self.multi_query_cross_att1 = CrossAttentionBlock_multi_query(embed_dim=256, num_heads=8)
        self.multi_query_cross_att2 = CrossAttentionBlock_multi_query(embed_dim=256, num_heads=8)
    ...
    def forward(self, key_frame, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None, training=True):
        ...
        features_image = features_image.view(b, -1, D) # B,T,D
        features_image = self.mamba_image(features_image)
        bboxes = self.proj_boxes(boxes)
        features_text = self.proj_text2(features_text)

        output = self.multi_query_cross_att1(features_image, bboxes, agg_trajs, features_text).mean(1)
        output_2 = self.multi_query_cross_att2(features_image, bboxes, agg_trajs, features_text).mean(1)

        residual_boxes = self.bbox_regression_head(output_2)
        ...
        if training:
            ...
            loss = 2*torch.mean(loss_bbox) + torch.mean(loss_cls) + 2*torch.mean(loss_bbox2) + torch.mean(loss_cls2) #+ mmd_loss  
